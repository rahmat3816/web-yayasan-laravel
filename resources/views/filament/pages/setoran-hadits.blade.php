...
@if(!empty($forbidden))
    <div class="p-6 bg-white dark:bg-gray-900 rounded-xl shadow-sm border border-gray-200 dark:border-gray-800 text-center space-y-3">
        <p class="text-lg font-semibold text-gray-800 dark:text-gray-100">Maaf, halaman ini khusus untuk Pengampu Tahfizh Hadits.</p>
...
<div class="setoran-wrapper">
    <section class="setoran-hero">
        <div class="setoran-hero__content">
            <div class="space-y-3">
                <p class="setoran-eyebrow">Tahfizh Hadits</p>
                <h1 class="text-3xl md:text-4xl font-semibold leading-tight">Setoran Hafalan Hadits</h1>
            ...
            <div class="setoran-hero__form">
                <form method="GET" action="{{ route('filament.admin.pages.tahfizh.hadits.setoran') }}" class="space-y-3">
    ...
    <section class="setoran-card text-slate-900 dark:text-white">
        <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
                <h2 class="text-lg font-semibold">Santri Tahfizh Hadits</h2>
                <p class="text-sm text-slate-500 dark:text-slate-300">Kelola setoran per santri berdasarkan target hadits.</p>
            </div>
            <a href="{{ route('filament.admin.resources.hadits-setorans.index') }}"
               class="setoran-link">
                <x-filament::icon icon="heroicon-o-clipboard-document" class="h-5 w-5" />
                Kelola Setoran Detail
            </a>
        </div>
            ...
            @foreach ($santriList as $s)
                <article class="santri-card">
                    ...
                    <dl class="mt-5 space-y-2 text-sm">
                        ...
                        <div class="flex items-center justify-between">
                            <dt class="text-slate-500 dark:text-slate-300">Target Hadits</dt>
                            <dd class="font-semibold">{{ $s->hadits_targets_count ?? 0 }}</dd>
                        </div>
                        <div class="flex items-center justify-between">
                        ...
                        <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                            <dt>Terakhir</dt>
                            <dd>{{ $cardSetoranMap[$s->id]['last_date'] ?? '-' }}</dd>
                        </div>
                    </dl>
                    <div class="mt-6">
                        <a href="{{ route('filament.admin.pages.hadits-setorans.create', ['santri_id' => $s->id]) }}"
                ...
                <tbody>
                    @forelse ($recentSetorans as $setoran)
                        <tr>
                            ...
                            <td>{{ $setoran->target->hadits->kitab ?? '-' }}</td>
                            <td>
                                <span class="font-semibold">Hadits {{ $setoran->target->hadits->nomor ?? '-' }}</span>
                                <p class="text-xs text-slate-500 dark:text-slate-300">{{ $setoran->target->hadits->judul ?? '-' }}</p>
                            </td>
    ...
    @if ($selectedSantri)
        <section class="setoran-card text-slate-900 dark:text-white">
            <div class="space-y-2">
                <h2 class="text-lg font-semibold">Target Hadits — {{ $selectedSantri->nama }}</h2>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="font-semibold text-slate-900 dark:text-white">
                                        Hadits {{ $summary['hadits_awal']['nomor'] ?? '-' }}
                                    </div>
                                    <p class="text-xs text-slate-500 dark:text-slate-300">
                                        {{ $summary['hadits_awal']['judul'] ?? '-' }}
                                    </p>
                                </td>
                                <td>
                                    <div class="font-semibold text-slate-900 dark:text-white">
                                        Hadits {{ $summary['hadits_akhir']['nomor'] ?? '-' }}
                                    </div>
                                    <p class="text-xs text-slate-500 dark:text-slate-300">
                                        {{ $summary['hadits_akhir']['judul'] ?? '-' }}
                                    </p>
                                </td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="5" class="text-center py-6 text-slate-500">Belum ada target hadits.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
