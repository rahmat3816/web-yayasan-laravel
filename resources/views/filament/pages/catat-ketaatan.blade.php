<x-filament::page class="setoran-page">
    <div class="setoran-wrapper space-y-8 max-w-5xl mx-auto w-full">
        <section class="setoran-hero rekap-hero mutun-hero">
            <div class="setoran-hero__content w-full">
                <div class="space-y-3">
                    <p class="setoran-eyebrow">Keamanan</p>
                    <h1 class="text-3xl md:text-4xl font-semibold leading-tight text-white">Catat Ketaatan Santri</h1>
                    <p class="text-sm md:text-base text-white/85 max-w-2xl">
                        Pilih santri, pilih ketaatan sebagai pengurang poin pelanggaran, poin terisi otomatis.
                    </p>
                </div>
            </div>
            <div class="hero-legend">
                <div>
                    ...
                    <p class="text-xs text-white/70">Pengurang poin tidak menurunkan level SP yang sudah dicapai.</p>
                </div>
            </div>
        </section>
        ...
        <form method="POST" action="{{ route('filament.admin.pages.keamanan.catat-ketaatan') }}" class="planner-form space-y-6">
            @csrf
            <div class="setoran-card form-shell space-y-5">
                <div class="form-header">
                    <div>
                        <p class="text-sm text-white/70">Form Ketaatan</p>
                        <h3 class="text-xl font-semibold text-white">Input Ketaatan Santri</h3>
                        <p class="text-xs text-white/60">Pilih santri dan ketaatan, poin otomatis terisi.</p>
                    </div>
                ...
                <div class="planner-form-grid">
                    ...
                    <div class="form-control">
                        <label class="field-label">Jenis Ketaatan</label>
                        <select name="ketaatan_type_id" id="ketaatanSelect" class="setoran-select" required>
                            <option value="">-- Pilih Ketaatan --</option>
                            @foreach ($ketaatanOptions as $opt)
                                <option value="{{ $opt->id }}" data-poin="{{ $opt->poin_pengurang ?? 0 }}">
                                    {{ $opt->nama }}
                                </option>
                            @endforeach
                        </select>
                        @error('ketaatan_type_id') <p class="field-error">{{ $message }}</p> @enderror
                    </div>
                    <div class="form-control">
                        <label class="field-label">Poin Pengurang</label>
    ...
    @push('scripts')
        <script>
            document.addEventListener('DOMContentLoaded', () => {
                const select = document.getElementById('ketaatanSelect');
