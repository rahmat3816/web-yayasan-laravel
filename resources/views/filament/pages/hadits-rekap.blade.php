...
<x-filament::page class="setoran-page">
    <div class="setoran-wrapper space-y-10">
        <section class="setoran-hero">
            <div class="setoran-hero__content w-full">
                <div class="space-y-4">
                    ...
                    <h1 class="text-3xl md:text-4xl font-semibold leading-tight">Rekap Target &amp; Setoran Hadits</h1>
                    <p class="text-white/80 max-w-2xl">
                        Pantau perkembangan target hafalan hadits berdasarkan unit, santri, dan kitab dengan tampilan yang konsisten dengan panel tahfizh Qur'an.
                    </p>
                    <div class="flex flex-wrap gap-3 text-sm font-semibold">
                        <span class="setoran-pill">Unit: {{ $unitLabel }}</span>
                        <span class="setoran-pill">Tahun: {{ $tahun ?? now()->year }}</span>
                ...
                <div class="setoran-hero__form">
                    <div class="grid gap-4 md:grid-cols-2">
                        ...
                        <label class="text-xs uppercase tracking-wide text-white/80 font-semibold flex flex-col gap-2">
                            <span>Pilihan Kitab Hadits</span>
        ...
        @if (!empty($kitabSummary))
            <section class="setoran-card hadits-kitabs space-y-4">
                <div class="rekap-table__header">
                    <div>
                        <h2 class="text-xl font-semibold">Ringkasan Per Kitab</h2>
                        <p class="text-sm">6 kitab teratas berdasarkan rata-rata progres.</p>
                    </div>
                </div>
                <div class="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                    @foreach ($kitabSummary as $kitab => $summary)
                        <article class="kitab-card">
                            ...
                            <div class="kitab-card__meta">
                                <span>Total: <strong>{{ number_format($summary['total'] ?? 0) }}</strong></span>
                                <span>Selesai: <strong>{{ number_format($summary['completed'] ?? 0) }}</strong></span>
                            </div>
                            <div class="kitab-card__progress">
                                <div class="kitab-card__progress-bar" style="width: {{ min(100, $summary['avg_progress'] ?? 0) }}%"></div>
                            </div>
                        </article>
                    @endforeach
                </div>
        ...
        <section class="setoran-card rekap-table">
            <div class="rekap-table__header">
                <div>
                    <h2>Detail Setoran Hadits</h2>
                    <p>Rekap setoran yang telah dilakukan berdasarkan filter yang dipilih.</p>
                </div>
            </div>
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="font-semibold text-sm text-slate-800">
                                        Hadits {{ $record['nomor'] ?? '-' }} • {{ $record['hadits'] }}
                                    </div>
                                    <p class="text-xs text-slate-500">{{ $record['kitab'] }}</p>
                                </td>
                                <td class="text-sm text-slate-600">{{ $record['penilai'] }}</td>
                                <td class="text-lg font-semibold text-slate-800">{{ $record['mutqin'] ?? '-' }}</td>
                                <td class="text-sm text-slate-600">{{ $record['tanggal'] ?? '-' }}</td>
                            </tr>
                        @empty
                            <tr>
                                <td colspan="6" class="rekap-table__empty">
                                    Belum ada data setoran hadits untuk filter yang dipilih.
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
