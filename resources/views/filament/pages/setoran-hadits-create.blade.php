...
<div class="setoran-create space-y-8 max-w-5xl mx-auto w-full">
    <section class="setoran-create__hero mb-6">
        <div class="space-y-4">
            <p class="setoran-create__eyebrow">Input Setoran Hadits</p>
            <h1 class="text-3xl md:text-4xl font-semibold leading-tight text-white">
                Tambah Setoran Hafalan Hadits
            </h1>
    ...
    <div class="grid gap-6 lg:grid-cols-3">
        <div class="lg:col-span-2 setoran-panel space-y-6">
            <div class="flex items-center justify-between gap-4">
                <div>
                    <h2 class="text-xl font-semibold text-slate-900 dark:text-white">Form Setoran Hadits</h2>
                    <p class="text-sm text-slate-500 dark:text-slate-400">Pilih target hadits yang disetorkan lalu isi nilai penilaian.</p>
                </div>
                <a href="{{ route('filament.admin.pages.tahfizh.hadits.setoran') }}"
            ...
            <form method="POST"
                  action="{{ route('filament.admin.pages.hadits-setorans.store') }}"
                  class="space-y-6">
                @csrf
                ...
                <div class="planner-form-grid">
                    <div class="form-control span-2">
                        <label class="field-label">Target Hadits</label>
                        <select name="target_id"
                            ...
                            @foreach ($targets as $target)
                                ...
                                <option value="{{ $target->id }}"
                                    ...
                                    Hadits {{ $target->hadits->nomor ?? '-' }} — {{ $target->hadits->judul ?? '-' }}
                                    @if($isCompleted)
                                        (Sudah setor)
                                    @endif
                                </option>
                        ...
                        @if($targetSelectionDisabled)
                            <p class="mt-2 text-sm text-amber-600 dark:text-amber-400">
                                Semua target hadits untuk santri ini telah disetorkan.
                            </p>
                        @endif
                ...
                <div class="field-group">
                    ...
                    <select name="nilai_mutqin" class="setoran-select" required>
                        ...
                        @for ($i = 1; $i <= 10; $i++)
                            <option value="{{ $i }}" @selected((int) old('nilai_mutqin') === $i)>{{ $i }}</option>
                        @endfor
                    </select>
                ...
                <div class="field-group">
                    ...
                    <textarea name="catatan" rows="3" class="setoran-textarea"
                ...
                <div class="flex flex-wrap items-center justify-between gap-3 pt-2">
                    <a href="{{ route('filament.admin.pages.tahfizh.hadits.setoran') }}" class="setoran-secondary-btn">
                        Batalkan
                    </a>
                    <button type="submit" class="setoran-primary-btn" @disabled($targetSelectionDisabled)>
                        Simpan Setoran
                    </button>
                </div>
            </form>
        ...
        <aside class="setoran-sidebar space-y-5">
            <div class="sidebar-note">
                Nilai mutqin mengikuti standar penilaian tahfizh hadits.
            </div>
            <div class="planner-meta">
                <span class="planner-meta__label">Total Target</span>
                <span class="planner-meta__value">{{ $targets->count() }}</span>
            </div>
        </aside>
    </div>
</div>
